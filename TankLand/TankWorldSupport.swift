...
extension TankWorld{
    ...
    func dealMineDamage(_ mine:Mine,_ go:GameObject){
        let dmg=mine.energy*Constants.mineStrikeMultiple
        mine.addEnergy(amount: -mine.energy)
        grid[mine.position.row][mine.position.col]=nil
        dealExplosionDamage(dmg,go);
    }
    func dealExplosionDamage(_ dmg:Int,_ go:GameObject){
        if(go.objectType == .Tank){
            let tank=go as!Tank
            if(tank.shields>dmg){
                tank.setShields(amount: tank.shields-dmg)
            }else{
                let rdmg=dmg-tank.shields
                tank.setShields(amount: 0)
                go.addEnergy(amount: -rdmg)
            }
